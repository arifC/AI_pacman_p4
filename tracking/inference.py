...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # ghost in jail --> update belief to 1
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            # new_p is a tuple containing the probabilities
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            for par in range(len(self.particles)):
                self.particles[par] = self.getJailPosition()
        
        else:
            newBeliefs = util.Counter()
            for pos in self.legalPositions:
                distance = util.manhattanDistance(pacmanPosition, pos)
                newBeliefs[pos] = emissionModel[distance] * self.beliefs[pos]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        allCombinations = itertools.product(self.legalPositions, self.legalPositions)
        
        for i in range(self.numParticles):
            for position in allCombinations:
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    updatedBelief *= emissionModels[i][distance]
            newBeliefs[p] += updatedBelief

        # all particle weights are 0
        if not any(newBeliefs.values()):
            # recreation of distribution
            self.initializeParticles()
            for p in self.particles:
                for i in range(self.numGhosts):
                    # ghost in jail
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                newPosDist = getPositionDistributionForGhost(newGhostPositions, i, self.ghostAgents[i])
